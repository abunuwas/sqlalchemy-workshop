...
from pyjobs.web.api.schemas import (
    GetJobSchema,
    ListJobsSchema,
    CreateJobSchema,
    ContractTypeEnum,
...
async def get_skills(
    ...
    with session_maker() as session:
        ...
        sorting_order = (
            desc if sortByOrder.value == SortByOrder.descending.value else asc
        )
...
async def get_locations(
    ...
    with session_maker() as session:
        ...
        locations = query.order_by(sorting_order(Location.country)).order_by(
            sorting_order(Location.city)
        )
        offset = (page - 1) * perPage if page > 1 else None
    return {
        "locations": [
            location.dict() for location in locations.limit(perPage).offset(offset)
        ],
...
async def apply_for_job(job: CreateJobApplication):
    with session_maker() as session:
...
async def list_applications(
    ...
    return {
        "applications": [
            application.dict()
            for application in applications.limit(perPage).offset(offset)
        ],
...
async def get_jobs(
    ...
    with session_maker() as session:
        ...
        sort_by = (
            getattr(Job, "date_posted")
            if sortBy == SortByEnum.datePosted
            else getattr(Job, "rate_annualized")
        )
...
async def create_job(job_details: CreateJobSchema):
    with session_maker() as session:
        job = Job(
            ...
            skills=session.query(Skill).filter(Skill.id.in_(job_details.skills)).all(),
        )
        session.commit()
        return job.dict()
...
async def update_job(job_id: int, job_details: CreateJobSchema):
    with session_maker() as session:
        ...
        job.skills = session.query(Skill).filter(Skill.id.in_(job_details.skills)).all()
        session.commit()
        return job.dict()


@router.delete(
    "/jobs/{job_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response
)
