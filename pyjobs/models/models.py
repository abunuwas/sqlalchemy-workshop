...
class Location(Base):
    ...
    __table_args__ = (UniqueConstraint("city", "country", name="city_country_constraint"),)
...
class Skill(Base):
    __tablename__ = "skill"
    __table_args__ = (UniqueConstraint("name", name="skill_name_constraint"),)
    ...
    def __str__(self):
        return f"<Skill: id={self.id}, created={self.created}, updated={self.updated}, name=self.{self.name}>"
